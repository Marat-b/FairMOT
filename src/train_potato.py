    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if opt.val_intervals > 0 and epoch % opt.val_intervals == 0:
            ...
            save_model(os.path.join(opt.save_dir, f'model_last{get_ymd()}.pth'),
                       epoch, model, optimizer)
            shutil.copy(
                os.path.join(opt.save_dir, f'model_last{get_ymd()}.pth'),
                os.path.join(opt.output_dir, f'fairmot_model_last{get_ymd()}.pth')
            )
        logger.write('\n')
        ...
        if epoch % 5 == 0 or epoch >= 25:
            ...
            shutil.copy(
                ...
                os.path.join(opt.output_dir, 'fairmot_model{}.pth'.format(get_ymd()))
            )
    logger.close()
